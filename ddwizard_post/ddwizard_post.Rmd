...
On the `DIAGNOSE` tab, users can further:
...
At its core, DDWizard was conceived as a tool to make it easier for researchers and practioners to communicate design properties. In line with this, users are able to visualize, download, and share their designs and diagnostic properties. Additionally, users can also share the status of the DDWizard by using the url provided by the `Share` button on top right corner. Value inputs and simulations are cached and loaded upon re-entering the token link on the browser.
...
text1 <- c(
  add_title("Load a design template", 
            "Select and load the 'Multi-arm' design. To learn about this design template in more detail, click on <code>Read more</code>. <p> <p> The <code>?</code> button will redirect to the full database of design templates."),
...
text3 <- c(
  add_title("Create a plot", 
            "Select the estimator and term to be displayed, the parameter to appear on the x-axis and any other parameters to be displayed in the color aesthetics (optional) or along different plots (optional)."),
