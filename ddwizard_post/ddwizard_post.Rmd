...
[DDWizard](https://eos.wzb.eu/ipi/DDWizard) is a web app interface that allows users to apply the [_DeclareDesign_ framework](https://declaredesign.org/declare.pdf) to easily define, customize, and examine designs from an existing library of templates (available in the R package [_DesignLibrary_](https://cran.r-project.org/web/packages/DesignLibrary/DesignLibrary.pdf)). It is a tool built for researchers and practicioners who have different levels of familiarity with R and/or _DeclareDesign_ packages.

By loading a design in the `DESIGN` tab, users can:

* customize design parameters (`Set design parameters` panel);
...
In the `DIAGNOSE` tab, users can compare the effect of different design parameters on a variety of diagnostic properties such as power or bias. Specifically, users may:

* vary design parameters (`Compare design parameters` panel);
* customize the number of simulations used in the design diagnosis (`Configure simulations` panel); 
* graph a comparison of design properties (`Diagnostic plots` panel) by selecting specific estimators, design parameters, and properties of interest, or diagnosands, (`Plot configuration` panel);
* visualize and download a table with diagnostic properties of designs (`Diagnosis` panel)

<!-- These features allow researchers to better communicate trade-offs in design choices and visualize, download, and share diagnostic statistics of those designs. -->

At its core, DDWizard was conceived as a tool to make it easier for researchers and practioners to communicate design properties. In line with this, users are able to visualize, download, and share their designs and diagnostic properties. Additionally, users can share the current state of the DDWizard app by using the URL provided by the `Share` button on top right corner. When someone else (e.g. a co-author) visits this URL, the app's state will be completely restored including design parameters, simulation data and diagnostic outputs.

### Five Practical Examples

Below we guide users on how to use these features on their daily research through five examples.
...
text1 <- c(
  ...
  add_title("Set values of design parameters", 
    ...
    add_title("Fix parameter", 
            "When creating a design, users can specify whether they want to hard code any or all parameters in design code. This means that the R code exported will prevent other users to easily redefine the values of these parameters. Any parameters set to 'fixed' will also be omitted from design diagnosis."),
...
3. Download _DDWizard_ [here](https://github.com/DeclareDesign/DDWizard) and run `runApp()` on the app directory.
